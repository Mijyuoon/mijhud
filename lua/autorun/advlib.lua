...
function loadmodule(name, ...)
	local nocheck = false
	if name[1] == "#" then
		name, nocheck = name:sub(2), true
	end
	if not nocheck and MODLOAD[name] then
		return MODLOAD[name]
	end
	local kname = name:gsub("%.", "/") .. ".lua"
	local is_sv = file.Exists(kname, "LSV")
...
function IncludeDir(dir)
	...
	for _, fn in ipairs(file.Find(dir.."*", realm)) do
		if not fn:find("%.lua$") then continue end
		out[fn] = include(dir..fn)
	end
	return out
end
...
local function error_no_halt(msg)
	for i = 1, #msg, 511 do
		ErrorNoHalt(msg:sub(i, i+510))
	end
end

function adv.ErrorSafe(func, ...)
	local f, a, b, c, d = xpcall(func, debug.traceback, ...)
	if f then return a, b, c, d end
	error_no_halt(a.."\n")
end

do ---- Add type marker to Color metatable ----
	local color_mt = FindMetaTable("Color")
	color_mt.__type = "Color"
end -------------------------------------------

function adv.Type(val)
	local tval = type(val)
	if tval == "table" then
		local mt = getmetatable(val)
		if istable(mt) and mt.__type then
			return mt.__type
		end
	end
	return tval
...
function adv.CheckArg(val, idx, tdef)
	local tval = adv.Type(val)
	for ts in tdef:gmatch("%S+") do
		if tval == ts then return end
	end
	local fn = debug.getinfo(2, "n").name
	error(adv.StrFormat{"bad argument #$1 to '$2' ($3 expected, got $4)",
		idx, fn or "?", tdef:gsub("%s+","|"), tval}, 3)
end

local t_empty = {}

function adv.Accessor(tbl, varname, name, opts)
	opts = opts or t_empty
	...
	if not opts.readonly then
		...
		local convfunc = opts.convfunc
		local dirtyvar = opts.dirtyvar
		tbl["Set"..name] = function(self, value)
			if check then
				adv.CheckArg(value, 1, check)
			end
			if default and value == nil then
				value = default
			end
			if convfunc then
				value = convfunc(self, value)
			end
			if dirtyvar and self[varname] ~= value then
				self[dirtyvar] = true
			end
			self[varname] = value
		end
...
function adv.Pairs(tbl)
	local mt = getmetatable(tbl)
	if istable(mt) and mt.__pairs then
		return mt.__pairs(tbl)
	end
	return next, tbl
end

function adv.ROTable(tbl)
	return setmetatable({}, {
		__type = "ROTable",
		__index = tbl,
		__newindex = function() end,
		__pairs = adv.FnBind1(pairs, tbl),
	})
end
...
function adv.StrTrimIndent(str, tabw)
	tabw = tabw or adv.TAB_WIDTH
	local indent, lines = 1e33, {}
	for line in str:gmatch("[^\n]+") do
		lines[#lines+1] = line
		local lpos = str:find("%S")
		if not lpos then continue end
		local nn = 0
		for i = 1, lpos do
			local cn = line:byte(i)
			nn = nn + (cn == 0x09 and tabw or 1)
		end
		indent = math.min(nn, indent)
	end
	if indent == 1e33 then return str end
	for	i = 1, #lines do
		local line = lines[i]
		local pos, nn = 0, indent
		while nn > 0 do
			local cn = line:byte(pos)
			nn = nn - (cn == 0x09 and tabw or 1)
			pos = pos + 1
		end
		lines[i] = line:sub(pos)
	end
...
function adv.TblRemoveI(tbl, value)
	for i = #tbl, 1, -1 do
		if tbl[i] == value then
			table.remove(tbl, i)
		end
...
function adv.FnBind1(fn, a1)
    return function(...)
        return fn(a1, ...)
    end
end

function adv.FnBind2(fn, a1, a2)
    return function(...)
        return fn(a1, a2, ...)
    end
end

function adv.FnBind3(fn, a1, a2, a3)
    return function(...)
        return fn(a1, a2, a3, ...)
    end
end

function adv.FnBind4(fn, a1, a2, a3, a4)
    return function(...)
...
do ---- Data structure networking --------------------
	...
	function ns_meta:__CstmFunc(var, id)
	...
	function ns_meta:__Iterate(node, ni)
		...
		if ntyp == "!!" then
			local par = mod and "select(2,...)" or ""
			self:__Append("local _val$1 = {$2}", { lvl, par })
			self:__Append("local _ctx = ...")
		elseif ntyp == "#" then
		...
		for i = 2, #node, step do
			...
			elseif val[1] == "*" then
				...
				local vn = adv.StrFormat{"_val$1[$2]", lvl, key}
				if mod then
					if not self.Writers[fnc] then
						error("unknown writer: *"..fnc)
					end
					self:__Append("_ctx.Writers['$1']($2)", {fnc, vn})
				else
					if not self.Readers[fnc] then
						error("unknown reader: *"..fnc)
					end
					self:__Append("$2 = _ctx.Readers['$1']()", {fnc, vn})
				end
			else
	...
	function ns_meta:InitWriter()
		...
		self:__Iterate(self.Descr)
		local code = table.concat(self.__Buffer, "\n")
		self.WriteFunc = CompileString(code, "NSr")
		self.__Buffer, self.__Prefix = nil
		self.__Mode, self.__Lvl = nil
		return true
	end
	
	function ns_meta:InitReader()
		if self.ReadFunc then
			return false
		end
		self.__Lvl = 0
		self.__Buffer = {}
		...
		self:__Iterate(self.Descr)
		local code = table.concat(self.__Buffer, "\n")
		self.ReadFunc = CompileString(code, "NSw")
		self.__Buffer, self.__Prefix = nil
		self.__Mode, self.__Lvl = nil
		return true
	end
